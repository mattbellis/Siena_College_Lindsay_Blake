...
def Gaussian(x,mean,width):
    ...
    ################ I MADE A MISTAKE IN THE WIDTH IN THE DENOMINATOR! SORRY! ########################
...
#width can range from a value of zero to twenty
    ...
    ####################### DO THIS OVER A SMALLER RANGE BUT WITH SMALL STEPS ALSO #############
    for width in np.arange(2,10,0.2):
        ...
        for x in range(Nstudents):
            score = scores[x]
            my_gauss = Gaussian(score, ps_mean, ps_width)
            logd = 0.0
            if my_gauss>0:
